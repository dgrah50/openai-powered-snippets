...
import getCommandRunnerContext, {
  CommandRunnerContext,
} from "./core/command-runner-context";
...
function initVsCodeCommands(context: vscode.ExtensionContext) {
  const commandExplain = vscode.commands.registerCommand(
    "ai-powered-snippets.run",
    async () => {
      commandRunnerContext.setSystemVariable(
        new Variable(systemVariableNames.extensionUri, context.extensionUri)
      );

      let selectedCommand = await vscode.window.showQuickPick<{
        label: string;
        description: string;
        command: Command;
      }>(
        commandRunnerContext.getCommands().map((c) => ({
          label: c.name,
...
function initConfiguration() {
  vscode.workspace.onDidChangeConfiguration(
    (event: vscode.ConfigurationChangeEvent) => {
      if (event.affectsConfiguration('"ai-powered-snippets.openAIToken')) {
        const config = vscode.workspace.getConfiguration("ai-powered-snippets");
        setOpenAIApiKey(config.get("openAIToken") as string | undefined);
        if (!config.get("openAIToken")) {
          log.error("Open AI api key required");
        }
      } else if (
        event.affectsConfiguration("ai-powered-snippets.snippetFiles")
      ) {
        const config = vscode.workspace.getConfiguration("ai-powered-snippets");
  ...
  return vscode.workspace.getConfiguration("ai-powered-snippets");
}
